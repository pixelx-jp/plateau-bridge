...
plateau-bridge is organised as **layers**, not modules-by-feature. Each layer has
...
and `src/plateau_bridge/ops/geometric_height.py` for the geometric
back-fill mechanics.

## D2 — Geometric height extraction must respect interleaved bufferViews

`src/plateau_bridge/ops/geometric_height.py` reads vertex
...
`src/plateau_bridge/ops/building_complex.py` runs in gate_b and writes
...
`src/plateau_bridge/catalog_registry.json` is the only source of truth
